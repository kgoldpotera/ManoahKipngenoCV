<script lang="ts">
	...
	const skills = [
		{ category: 'Frontend', items: ['SvelteKit', 'Svelte 5 Runes', 'TypeScript', 'Tailwind CSS', 'GSAP', 'D3.js', 'HTML5/CSS3'] },
		{ category: 'Backend & Data', items: ['Python', 'Supabase', 'IndexedDB / Dexie.js', 'Fuse.js', 'REST APIs'] },
		{ category: 'Tools & Platforms', items: ['Git / GitHub', 'Vercel', 'Vite', 'Tauri', 'VS Code', 'Figma'] },
		{ category: 'Soft Skills', items: ['Quick Learner', 'Problem Solving', 'Technical Writing', 'Team Collaboration', 'Process Design'] }
	];
	...
	const timeline = [
		{
			...
			description: 'Architected front-end data visualization tools, built zero-latency search, and designed error handling systems for East Africa\'s premier digital medical directory.',
			current: true
		},
		{
			period: '2022 — Present',
			...
			description: 'Core focus: bridging modern software engineering with telecommunications infrastructure. Strong foundation in C, Python, and systems design.',
			current: true
		},
		{
			...
			description: 'Led outreach to 100+ clients, built partner databases, and designed a 48-hour job placement system.',
			current: false
		},
		{
			period: '2018 — 2022',
			...
			description: 'Strong foundation in sciences and mathematics, establishing rigorous analytical skills required for engineering.',
			current: false
		}
	];
	...
	<div class="mx-auto max-w-5xl">
		...
		<div class="info-grid mb-20 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
			...
			<div class="info-card glass-card p-5">
				...
				<a href="tel:+254706487376" class="mt-1 block font-semibold text-white transition-colors hover:text-cyan-400">+254 706 487 376</a>
			</div>
			<div class="info-card glass-card p-5">
				<span class="text-xs font-bold uppercase tracking-widest text-slate-500">Email</span>
				<a href="mailto:koechmanoah32@gmail.com" class="mt-1 block font-semibold text-white transition-colors hover:text-cyan-400">koechmanoah32@gmail.com</a>
			</div>
			<div class="info-card glass-card p-5">
				<span class="text-xs font-bold uppercase tracking-widest text-slate-500">GitHub</span>
				<a href="https://github.com/kgoldpotera" target="_blank" rel="noopener noreferrer" class="mt-1 block font-semibold text-white transition-colors hover:text-cyan-400">kgoldpotera</a>
			</div>
		</div>
			...
			<div class="grid gap-6 md:grid-cols-2">
				{#each skills as group}
					<div class="skill-group glass-card p-6">
						<h3 class="mb-4 text-sm font-bold uppercase tracking-widest text-cyan-400">{group.category}</h3>
				...
				{#each timeline as item}
					<div class="timeline-item glass-card flex gap-6 p-6">
						<div class="hidden flex-shrink-0 flex-col items-center sm:flex">
							<div class="h-3 w-3 rounded-full {item.current ? 'bg-cyan-400 shadow-lg shadow-cyan-400/30' : 'bg-slate-600'}"></div>
							<div class="mt-2 h-full w-px bg-white/10"></div>
						</div>
						<div class="flex-1">
							<div class="mb-1 flex flex-wrap items-center gap-3">
								<span class="text-xs font-bold uppercase tracking-widest text-slate-500">{item.period}</span>
								{#if item.current}
									<span class="rounded-full bg-cyan-500/10 px-2 py-0.5 text-[10px] font-bold text-cyan-400">Current</span>
								{/if}
							</div>
			...
			<div class="pub-grid grid gap-6 md:grid-cols-2">
				...
				>
					...
					<p class="mt-2 text-sm text-slate-400">
						Inside the engineering and design process behind modernizing Kenya's most trusted drug reference platform.
					</p>
				...
				>
					...
					<p class="mt-2 text-sm text-slate-400">
						Analyzing performance trade-offs and accessibility gaps in Apple's modern design language.
					</p>
					<span class="mt-4 inline-flex items-center gap-1 text-xs font-medium text-slate-500">
						9 min read
						<svg class="h-3 w-3 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
		...
		<div class="glass-card p-10 text-center">
			...
			<p class="mt-2 text-slate-400">Check out my case studies or read my cover letter for actualfood.</p>
			<div class="mt-6 flex flex-wrap justify-center gap-4">
				<a href="/work" class="rounded-full bg-cyan-500 px-6 py-3 text-sm font-bold text-white transition-all hover:bg-cyan-400 hover:shadow-lg hover:shadow-cyan-500/25">
					Case Studies
				</a>
				<a href="/cover-letter" class="rounded-full border border-white/15 bg-white/5 px-6 py-3 text-sm font-semibold text-slate-300 transition-all hover:border-white/25 hover:bg-white/10 hover:text-white">
					Cover Letter
				</a>
			</div>
		</div>
